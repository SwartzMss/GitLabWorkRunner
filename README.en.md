...
`timeout_seconds` is a full budget for each AI Review execution. A context-assisted execution starts one newly timed, independent diff-only fallback only when it returns `review_run_timeout`, `ai_request_timeout`, or `ai_tool_loop_timeout`. The fallback preserves the same `timeout_seconds`, `request_timeout_seconds`, batching policy, and `max_batches`, but exposes no context tools and never runs its own clean-confirmation pass or recursively starts a third execution. With `second_pass_on_clean = false`, worst-case elapsed time can approach `2 × timeout_seconds`. With it set to `true`, the initial context pass may complete clean, the confirmation may consume another full budget and time out, and the independent fallback may consume a third full budget, so the worst case can approach `3 × timeout_seconds`. Both bounds also include small archive preparation, cleanup, and publishing overhead. This behavior adds no configuration. Other AI errors and archive timeout, permission, HTTP, corrupt-ZIP, or filesystem failures are not eligible for this timeout fallback.
