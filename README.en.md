...
An identical tool call receives a compact cache reference and consumes no additional call or byte budget. When either budget is exhausted, the runner removes `read_file`, `search_code`, and `list_files`, leaves only `submit_review_findings`, and requires immediate final submission.
...
Keep `request_timeout_seconds` below the timeout enforced by the upstream load balancer or API gateway. When a context follow-up returns 504 or times out, the one retry removes context tools and forces final submission instead of replaying the exploratory request unchanged.
