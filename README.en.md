...
- Runtime environment variables include `GITLAB_WORK_RUNNER_SOURCE_DIR`, `GITLAB_WORK_RUNNER_TASK_DIR`, `GITLAB_WORK_RUNNER_OUTPUT_PATH`, and `GITLAB_WORK_RUNNER_COMMIT_SHA`, so scripts can locate the checked source directory and output file.
...
The repository includes a minimal script example: [examples/scripts/check_todo_tbd.py](examples/scripts/check_todo_tbd.py). It scans text files in the current working directory and fails when it finds `//TODO` or `//TBD`, printing file locations.
