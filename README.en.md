...
Linux release packages include the same `scripts/linux-background.sh`. When run from an extracted release package, the script prefers `gitlab-work-runner` and `gitlab-work-runner-dashboard` in the package root; in a source checkout it falls back to `target/release/`.
...
`max_tool_rounds` defaults to `0`, which means unlimited tool follow-up rounds. When set to a non-zero value, it limits how many per-batch "model requests context tools -> runner returns tool results" follow-up rounds are allowed. After the final allowed round, the runner still returns that round's allowed tool results, then removes `read_file`, `search_code`, and `list_files`, leaves only `submit_review_findings`, and requires immediate final submission. If the model still requests context tools, the runner drops context-tool conversation history and retries once with clean diff-only finalization.
...
An identical tool call receives a compact cache reference and consumes no additional call or byte budget. Because repeating it cannot add evidence, a cache hit immediately enters finalization. After a cache hit or when the round, call, or byte budget is exhausted, the runner removes context tools and requires final submission.
Logs include each tool call's tool name, argument summary, returned bytes, truncation, cache/budget status, batch index/count, cumulative tool round, cumulative call count, and cumulative result bytes. This makes it clear whether the model actually called `read_file`, `search_code`, or `list_files`.
...
AI Review splits MR diffs by diff chunks; oversized single-file diffs are split by unified diff hunks where possible. `max_batch_diff_bytes` controls the per-batch raw diff byte limit, and `max_batches` controls the maximum number of AI requests; `0` means unlimited batches.
